  ...
  levelUp(level) {
    ...
    for (const member of this.positioned) {
      ...
      member.character.attack += Math.floor(Math.max(
        ...
        member.character.attack * (1.8 - (1 - member.character.health / 100)))).toFixed(2);
      member.character.defence += Math.floor(Math.max(
        member.character.defence,
        member.character.defence * (1.8 - (1 - member.character.health / 100)))).toFixed(2);
    ...
    for (const member of newMembers) {
      member.attack = Math.floor(member.attack
         * (Math.pow(1.8 - (1 - member.health / 100)),(member.level - 1)));
