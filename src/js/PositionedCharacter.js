...
export default class PositionedCharacter {
  ...
  get stepCells() {
    ...
    for (let i = 1; i <= this.character.stepRadius; i += 1) {
      ...
      if (right % boardSize > positionLine && left < Math.pow(boardSize,2)) {
        stepsArray.push(right);
      }
      if (bottomRight % boardSize > positionLine && bottomRight < Math.pow(boardSize,2)) {
        stepsArray.push(bottomRight);
      }
      if (bottom < Math.pow(boardSize,2)) {
        stepsArray.push(bottom);
      }
      if (bottomLeft % boardSize < positionLine && bottomLeft < Math.pow(boardSize,2)) {
